...
<template>
  <section id="section2" class="grid gap-5 items-center py-10 pl-10 lg:grid-cols-2 xl:grid-cols-3 bg-servicios md:ml-[60px] md:rounded-s-[64px] lg:ml-[8%]">
        ...
        <div class="xl:col-start-2">
          <div class="grid items-baseline">
            <div class="grid gap-6 items-center justify-content-center md:grid-cols-2 md:max-w-[300px]">
              <!-- Cambiamos el span por un h2 -->
              <h2 class="sr-only">Tu Próximo Nivel Financiero</h2>
            ...
            <div>
              ...
              <a
                href="https://api.whatsapp.com/send?phone=+5533352073&amp;text=¡Hola!%20Me%20interesa%20recibir%20más%20información%20de%20sus%20productos"
                target="_blank"
                rel="noopener"
                class="flex items-center gap-3 bg-[#1AB0DE] text-white md:text-[1.335rem] w-fit rounded-full shadow-sm py-1 px-10 pr-2 md:gap-8">
    ...
    <div class="lg:col-span-1 xl:col-span-2 overflow-hidden" data-aos="zoom-in-left" data-aos-anchor-placement="center-bottom">
      <div class="relative">
        <div id="carousel" class="overflow-hidden overflow-y-hidden relative" @mouseenter="pauseAutoSlide" @mouseleave="startAutoSlide">
          <div ref="carouselTrack" id="producto-track" class="flex transition-transform duration-500 ease-in-out" :style="{ transform: transformStyle }">
            <div v-for="(producto, index) in productos" :key="producto.id" class="bg-white rounded-full p-2 producto-card flex-shrink-0 min-h-[597px] text-center">
              ...
              <div>
                <h3 :class="['text-center text-[1.502rem] uppercase lato-black mb-3', getTitleColorClass(index)]">
                  {{ producto.titulo }}
                </h3>
